...
const UserReport = () => {
  ...
  const handleSaveEdit = async (movementID) => {
    try {
      // Send PUT request to update
      const response = await fetch(`http://localhost:5137/movementdata/${movementID}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ ...editedData, wasEdited: true }),
      });
      ...
      const globalIndex = movementData.findIndex((item) => item.movementID === movementID);
      if (globalIndex !== -1) {
        newData[globalIndex] = updated;
  ...
  return (
    <div className="min-h-[75vh] p-4 md:p-8">
      <div className="flex flex-row items-center justify-between mb-4">
        ...
        <Link
          to={`/user-information/${userID}`}
          className="text-blue-600 font-base hover:underline text-sm md:text-base"
        >
          Back to Profile
        </Link>
      </div>
      ...
      <div className="overflow-x-auto shadow rounded-lg">
        <table className="w-full table-auto bg-white">
          <thead className="bg-gray-800 text-white text-left">
            <tr>
              {['Date', 'Time', 'Punch', 'Status', 'Place', 'Party', 'Purpose', 'Remark', 'Actions'].map(
                (header) => (
          ...
          <tbody className="text-gray-700 divide-y">
            {paginatedData.map((item, index) => {
              const { date, time } = formatDateTime(item.dateTime);
              const editableWindow = new Date() - new Date(item.dateTime) < 10 * 60 * 1000;
              return (
                <tr key={item.movementID}>
                  ...
                  <td className="py-2 px-4">
                    {editIndex === index ? (
                      <input
                        ...
                        onChange={(e) =>
                          setEditedData((prev) => ({ ...prev, remark: e.target.value }))
                        }
                  ...
                  <td className="py-2 px-4">
                    {editIndex === index ? (
                      <>
                        <button
                          onClick={() => handleSaveEdit(item.movementID)}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mt-4 space-y-4 md:space-y-0">
        ...
        <div className="space-x-2">
          <button onClick={handleDownloadExcel} className="px-4 py-2 bg-green-600 text-white rounded">
            Download Excel
          </button>
